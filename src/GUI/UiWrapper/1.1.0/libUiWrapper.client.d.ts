...
// 重要提醒：
// 此类型声明文件对应的 UiWrapper 的版本为 1.1.0
...
declare class UiNodeWrapper<NodeType> {
     ...
     */
    config<Key extends keyof NodeType>(key: Key, value: Key extends "uiScale" ? UiScale | number | null : NodeType[Key], options?: "offset" | "scale" | "all"): UiNodeWrapper<NodeType>;
    /**
    ...
    /**
     * 按名称查找子节点（支持深度查找），返回对应子节点打包器。节点名称可在编辑模式下的属性面板中查看。请注意，使用此方法进行查找会返回 UiNodeWrapper，在未找到时返回 null
     * @param name 子节点的名称
     */
